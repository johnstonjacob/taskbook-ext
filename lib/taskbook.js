...
class Taskbook {
    ...
    _getBoards() {
        ...
        Object.keys(_data).forEach(id => {
            boards.push(
                ..._data[id].boards.filter(x => boards.indexOf(x) === -1)
            );
        });
    ...
    calendar(datetime, task) {
        const taskString = task.join(' ');
        opn(`x-fantastical2://parse?add=1&s=${datetime} ${taskString}`, {
            wait: false
        }).catch(() =>
            console.log(
                'Error adding event to your calendar. The task was still set.'
            )
        );
    }
